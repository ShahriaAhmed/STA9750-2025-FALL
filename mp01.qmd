...
season_stats <- setNames(average_by_season$average_weekly_hours, average_by_season$season_clean)
```
...
average_by_season <- stranger_things_global |>
  ...
  group_by(season_clean) |>
  summarise(
    average_weekly_hours = mean(weekly_hours_viewed, na.rm = TRUE), #get the average weekly hours
) |>
  arrange(match(season_clean, c("Season 1", "Season 2", "Season 3", "Season 4", "None")))
...
weeks_spent_in_top10 <- stranger_things_global |>
  group_by(season_title) |>
  summarise(
    weeks_in_top10 = n()
  )
total_weeks_top10 <- sum(weeks_spent_in_top10$weeks_in_top10)
```
...
unique_countries_count <- stranger_things_countries |>
  summarise(countries = n_distinct(country_name)) |>
  pull(countries)
```
5. Lets go *global*: Stranger Things appeared in the top 10 for **`r format(unique_countries_count, big.mark = ",")`** countries
...
comparison_table <- top_english_shows |>
  mutate(
    ...
    average_weekly_hours = format(round(average_weekly_hours), big.mark = ",", scientific = FALSE),
  ...
  select(
    ...
    `Weeks in Top 10` = weeks_spent_in_top10,
    Seasons = seasons
  )
...
yearly_summary <- joined_data %>%
  ...
  summarise(
    ...
    average_weekly_views = mean(weekly_views, na.rm = TRUE),
...
titles_by_year_category <- GLOBAL_TOP_10 %>%
  ...
  group_by(category, year) %>%
  summarise(
    unique_names = n_distinct(show_title)) %>%
...
\
<ins>Follow along to see additional insights from our Netflix data. Let's take a look below.</ins>
\
```{r data cleanup}
#| echo: true
...
tv_debut_countries <- COUNTRY_TOP_10 |>
  ...
  summarise(countries_count = n_distinct(country_name)) |>
  arrange(desc(countries_count))

print(tv_debut_countries)
```
\
Thank you for following along as we look through Netflix data. The findings highlight Netflix’s continued impact on global viewing habits, with audiences embracing stories that cross languages and borders. As data continues to shape entertainment trends, Netflix remains committed to delivering compelling and diverse content to audiences worldwide. See you on the next release! 