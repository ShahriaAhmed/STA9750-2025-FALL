...
average_by_season <- stranger_things_global |>
  ...
  arrange(match(season_clean, c("Season 1", "Season 2", "Season 3", "Season 4", "Season Unknown")))
...
#filter for english shows in the global df
...
#sum and average the weeekly hours viewed
...
#plot average hours viewed by week
...
#filter for top show by category and sum total hours
top_show_by_category <- GLOBAL_TOP_10 %>%
  group_by(category, show_title) %>%
  summarise(
    total_hours = sum(weekly_hours_viewed, na.rm = TRUE)) %>%
...
#summarize total views and total hours viewed
...
#get total hours views and divide by runtime to estimate viewership
