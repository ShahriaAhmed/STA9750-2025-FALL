...
This is pretty dense. Below is an interactive chart where you can zoom in on specific trees and regions, with a sample of 50,000 trees.
```{r}
#| code-fold: true
#| code-summary: "Interactive tree map with plotly"
#| cache: true
#| message: false
#| warning: false

# function to create an interactive map of nyc trees
#
# parameters:
# districts: sf object containing city council district boundaries
# trees: sf object containing nyc street tree locations
# sample_size: optional number of trees to plot
#
# returns: an interactive chart
plot_nyc_trees_interactive <- function(districts, trees, sample_size = NULL) {
  library(ggplot2)
  library(dplyr)
  library(plotly)
  library(sf)
  ...
  # add hover text with useful info about each tree
  # check which columns actually exist in the data
  trees_to_plot <- trees_to_plot |>
    mutate(
      hover_text = paste0(
        "Species: ", ifelse(!is.na(genusspecies), genusspecies, "Unknown"), "<br>",
        "Condition: ", ifelse(!is.na(tpcondition), tpcondition, "Unknown")
      )
    )
  ...
  p <- ggplot() +
    ...
    labs(
      title = "NYC Street Trees by City Council District",
      subtitle = sprintf("showing %s trees - zoom and hover for details",
                        format(nrow(trees_to_plot), big.mark = ",")),
      x = "Longitude",
      y = "Latitude"
    )
  ...
  # tooltip shows custom hover text, you can zoom/pan to explore dense areas
  interactive_plot <- ggplotly(p, tooltip = "text") |>
    layout(
      # improve the hover label appearance
      hoverlabel = list(
        bgcolor = "white",
        font = list(size = 12)
      )
...
#call function
plot_nyc_trees_interactive(nyc_districts, nyc_trees, sample_size = 50000)
```
...
```{r}
#| code-fold: true
#| code-summary: "Most common species"
#| message: false
#| warning: false
library(sf)
library(dplyr)
...
```{r}
#| code-fold: true
#| code-summary: "Closest to Baruch campus"
#| message: false
#| warning: false
...
ggplot() +
  # base layer: district 22 boundary as light gray background
  geom_sf(data = results$stats$astoria_district, 
          fill = "gray95",   # very light fill
          color = "gray50",  # medium gray border
          size = 1) +
  
  # layer 2: existing healthy trees in green
  geom_sf(data = results$stats$district_22_trees_sf |> 
            filter(!(tpcondition %in% c("Dead", "Retired") | !is.na(stumpdiameter))),  # exclude dead/stumps
          aes(color = "Existing Trees"), 
          size = 0.5,    # small dots
          alpha = 0.6) +  # semi-transparent
  
  # layer 3: planting opportunities (dead trees and stumps) in orange
  geom_sf(data = results$stats$district_22_trees_sf |> 
            filter(tpcondition %in% c("Dead", "Retired") | !is.na(stumpdiameter)),  # only dead/stumps
          aes(color = "Planting Sites (Dead/Retired)"), 
          size = 1.5,    # larger dots so they stand out
          alpha = 0.8) +
  
  # layer 4: 500m buffer around ps 122q shown as dashed circle
  geom_sf(data = results$stats$ps122_buffer, 
          fill = NA,              # no fill, just outline
          color = "darkgreen",    # green to match school zone theme
          linetype = "dashed",    # dashed line for "area of focus"
          size = 1) +
  
  # define colors for the two tree categories
  scale_color_manual(
  ...
  # zoom in to just the buffer area
  ...
  theme(
    legend.position = "bottom",  # legend at bottom so it doesn't cover map
...
# prep data for the bar chart - add labels and highlight district 22
density_plot_data <- results$stats$density_comparison |>
  mutate(
    district_label = paste("District", CounDist),  # create readable labels
    is_target = CounDist == 22  # flag district 22 for highlighting
  )

# create horizontal bar chart comparing tree density
ggplot(density_plot_data, aes(x = reorder(district_label, tree_density),  # sort by density (lowest first)
                               y = tree_density,
                               fill = is_target)) +  # color based on whether it's district 22
  
  # create the bars
  geom_col() +
  
  # add text labels showing total tree count for each district
  geom_text(aes(label = format(tree_count, big.mark = ",")),  # add commas to numbers
            hjust = -0.1,  # nudge text to the right of bars
            size = 3) +
  
  # color district 22 orange, others gray
  scale_fill_manual(values = c("TRUE" = "#E69F00", "FALSE" = "gray70"),
                    guide = "none") +  # hide legend
  
  # flip to horizontal bars for easier label reading
  coord_flip() +
  ...
  labs(
    ...
    x = "",  # no label needed for districts
...
# bar chart showing stump counts across districts
ggplot(density_plot_data, aes(x = reorder(district_label, stump_count),  # sort bars by stump count
                               y = stump_count,
                               fill = is_target)) +  # highlight district 22 in orange
  
  # create the bars
  geom_col() +
  
  # add text labels showing exact counts
  geom_text(aes(label = stump_count), 
            hjust = -0.1,  # push labels slightly to the right of bars
            size = 3) +
  
  # color scheme: orange for district 22, gray for others
  scale_fill_manual(values = c("TRUE" = "#E69F00", "FALSE" = "gray70"),
                    guide = "none") +  # hide the legend since colors are self-explanatory
  
  # flip coordinates so bars go horizontal (easier to read district names)
  coord_flip() +
...
ggplot() +
  # first layer: district boundaries with different colors for each district
  geom_sf(data = results$stats$comparison_district_bounds, 
          aes(fill = factor(CounDist)), 
          alpha = 0.3,        # make them semi-transparent so stumps show through
          color = "gray30") + # darker borders to distinguish districts
  
  # second layer: all the stump locations as red dots
  geom_sf(data = results$stats$stumps_all_districts, 
          color = "red",      # red really pops against the district colors
          size = 0.8,         # small dots since there are many
          alpha = 0.6) +      # slight transparency to see overlap
  
  # use a nice color palette for the district fills
  ...
  theme(
    legend.position = "right",  # legend on the side so it doesn't cover the map
    plot.title = element_text(face = "bold", size = 12)
  )
```